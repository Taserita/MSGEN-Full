...
def assemble_atom_pair_feature(node_attr, edge_index, edge_attr):
    ...
    h_pair = torch.cat([h_row*h_col, edge_attr], dim=-1)    # (E, 2H) / (E, 3H)
    return h_pair

def assemble_atom_pair_feature_cat(node_attr, edge_index, edge_attr):
    h_row, h_col = node_attr[edge_index[0]], node_attr[edge_index[1]]
    h_pair = torch.cat([h_row, h_col, edge_attr], dim=-1)    # (E, 3H) / (E, 4H)
    return h_pair
