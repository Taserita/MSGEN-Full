...
from ..common import assemble_atom_pair_feature_cat
...
from ..encoder import SchNetEncoder, GINEncoder, ConditionalGINEncoder, get_edge_encoder
...
class DualEncoderEpsNetwork(nn.Module):
    ...
    def __init__(self, config):
        ...
        `output_mlp` takes a mixture of two nodewise features and edge features as input and outputs 
            gradients w.r.t. edge_length (out_dim = 1). original don't use cat
        """
        ...
        # self.grad_global_dist_mlp = MultiLayerPerceptron(
        #     3 * config.hidden_dim,
        #     [2 * config.hidden_dim, config.hidden_dim, config.hidden_dim // 2, 1], 
        #     activation=config.mlp_act
        # )
    ...
    def forward(self, atom_type, pos, bond_index, bond_type, batch, time_step, 
        ...
        # h_pair_global = assemble_atom_pair_feature_cat(
        #     node_attr=node_attr_global,
        #     edge_index=edge_index,
        #     edge_attr=edge_attr_global,
        # )    # (E_global, 3H)
        ...
        # h_pair_local = assemble_atom_pair_feature_cat(
        #     node_attr=node_attr_local,
        #     edge_index=edge_index[:, local_edge_mask],
        #     edge_attr=edge_attr_local[local_edge_mask],
        # )    # (E_local, 3H)
...
def cg_local_edge(edge_type):
    return edge_type.sum(dim=-1) > 0
...
# Stage-2 Conditional Encoder
class ConditionalDualEncoderEpsNetwork(nn.Module):
    ...
    def __init__(self, config):
        ...
        """
        self.edge_encoder_global = get_edge_encoder(config)
        self.edge_condition_global = get_edge_encoder(config)
        self.edge_encoder_local = get_edge_encoder(config)
        self.edge_condition_local = get_edge_encoder(config)
        # self.hidden_dim = config.hidden_dim
        '''
        timestep embedding
        '''
        ...
        self.encoder_global = SchNetEncoder(
            ...
            num_filters=config.hidden_dim,
            num_interactions=config.num_convs,
            edge_channels=self.edge_encoder_global.out_channels * 2,
            cutoff=config.cutoff,
            smooth=config.smooth_conv,
        )
        self.encoder_local = ConditionalGINEncoder(
            hidden_dim=config.hidden_dim,
            num_convs=config.num_convs_local,
        )
        ...
        Incorporate parameters together
        '''
        self.model_global = nn.ModuleList([self.edge_encoder_global, self.edge_condition_global, self.encoder_global, self.grad_global_dist_mlp])
        self.model_local = nn.ModuleList([self.edge_encoder_local,self.edge_condition_local, self.encoder_local, self.grad_local_dist_mlp])

        self.model_type = config.type  # config.type  # 'diffusion'; "dsm"

        if self.model_type == 'diffusion':
            # denoising diffusion
            # we also need previous beta_st, beta_ed, steps, change for different pipeline
            betas_p = get_beta_schedule(
                beta_schedule=config.beta_schedule,
                beta_start=1.e-7,
                beta_end=2.e-3,
            ...
            betas_p = torch.from_numpy(betas_p).float()
            self.betas_p = nn.Parameter(betas_p, requires_grad=False)
            alphas_p = (1. -betas_p).cumprod(dim=0)
            self.alphas_p = nn.Parameter(alphas_p, requires_grad=False)
            self.num_timesteps_p = self.betas_p.size(0)
            ...
            sigmas = torch.tensor(
                np.exp(np.linspace(np.log(config.sigma_begin), np.log(config.sigma_end),
                                config.num_noise_level)), dtype=torch.float32) # exp-log trick
            self.sigmas = nn.Parameter(sigmas, requires_grad=False) # (num_noise_level)
            self.num_timesteps = self.sigmas.size(0)  # betas.shape[0]
    
    def forward(self, atom_type, pos, pos_guide, bond_index, bond_type, batch, time_step, 
                edge_index=None, edge_type=None, edge_length=None, return_edges=False, 
                extend_order=True, extend_radius=True, is_sidechain=None):
        """
        Args:
        ...
        if edge_index is None or edge_type is None or edge_length is None:
            ...
            edge_length_guide = get_distance(pos_guide, edge_index).unsqueeze(-1) # (E, 1)
        ...
        if self.model_type == 'dsm':
            ...
            node2graph = batch
            edge2graph = node2graph.index_select(0, edge_index[0])
            # select node_2graph based on edge_index[0] -> we get each edge src node belong to which graph
            sigma_edge = noise_levels.index_select(0, edge2graph).unsqueeze(-1)  # (E, 1)
            # temb_edge = temb.index_select(0, edge2graph)  # (E , dim)
        elif self.model_type == 'diffusion':
            # with the parameterization of NCSNv2
            # DDPM loss implicit handle the noise variance scale conditioning (sample different time of noising for different graph)
            sigma_edge = torch.ones(size=(edge_index.size(1), 1), device=pos.device)  # (E, 1)

        # Encoding global edge features: dist + type -> emb
        edge_attr_global = self.edge_encoder_global(
            edge_length=edge_length,
            edge_type=edge_type
        )   # Embed edges

        edge_attr_guide_global = self.edge_condition_global(
            edge_length=edge_length_guide,
            edge_type=edge_type
        )
        # edge_attr += temb_edge
        f_edge_attr_global = torch.cat([edge_attr_global, edge_attr_guide_global], dim=-1) # [E, 2*hidden]

        # Global-cutoff + bond graph
        node_attr_global = self.encoder_global(
            z=atom_type,
            edge_index=edge_index,
            edge_length=edge_length,
            edge_attr=f_edge_attr_global,
        )
        ## Assemble pairwise features
        h_pair_global = assemble_atom_pair_feature(
            node_attr=node_attr_global,
        ...
        #     edge_attr=f_edge_attr_global,
        # )    # (E_global, 4H)
        ## Invariant features of edges (radius graph, global), noise-aware, From Learning Gradient field...
        ## They use a MLP to estimate the gradient of the distance distribution, for DDPM sigma_edge = 1
        edge_inv_global = self.grad_global_dist_mlp(h_pair_global) * (1.0 / sigma_edge)    # (E_global, 1)
        ...
        edge_attr_guide_local = self.edge_condition_local(
            edge_length=edge_length_guide,
            edge_type=edge_type
        )

        f_edge_attr_local = torch.cat([edge_attr_local, edge_attr_guide_local], dim=-1)
        # Local
        node_attr_local = self.encoder_local(
            z=atom_type,
            edge_index=edge_index[:, local_edge_mask],
            edge_attr=f_edge_attr_local[local_edge_mask],
        )
        ## Assemble pairwise features
        h_pair_local = assemble_atom_pair_feature(
            node_attr=node_attr_local,
    ...
    def get_loss(self, atom_type, pos, pos_guide, bond_index, bond_type, batch, num_nodes_per_graph, num_graphs, 
                 anneal_power=2.0, return_unreduced_loss=False, return_unreduced_edge_loss=False, extend_order=True, extend_radius=True, is_sidechain=None):
        return self.get_loss_diffusion(atom_type, pos, pos_guide, bond_index, bond_type, batch, num_nodes_per_graph, num_graphs, 
                anneal_power, return_unreduced_loss, return_unreduced_edge_loss, extend_order, extend_radius, is_sidechain)



    def get_loss_diffusion(self, atom_type, pos, pos_guide, bond_index, bond_type, batch, num_nodes_per_graph, num_graphs,
                 anneal_power=2.0, return_unreduced_loss=False, return_unreduced_edge_loss=False, extend_order=True, extend_radius=True, is_sidechain=None):
        N = atom_type.size(0)
        node2graph = batch

        # Four elements for DDPM: original_data(pos), gaussian_noise(pos_noise), beta(sigma), time_step
        # Sample noise levels - sample random time steps for each graph with balanced pairing
        time_step_p = torch.randint(
            0, self.num_timesteps_p, size=(num_graphs//2+1, ), device=pos.device
        )
        time_step_p = torch.cat(
            [time_step_p, self.num_timesteps_p-time_step_p-1], dim=0)[:num_graphs]
        ...
        a = self.alphas.index_select(0, time_step)  # (G, )
        a_p = self.alphas_p.index_select(0, time_step_p) # (G, )

        # perturb pos_guide for augmentation
        pre_pos = a_p.index_select(0, node2graph).unsqueeze(-1)
        pre_pos_noise = torch.zeros(size=pos_guide.size(), device=pos_guide.device)
        pre_pos_noise.normal_()
        pos_guide_perturbed = pos_guide + pre_pos_noise * (1.0 - pre_pos).sqrt() / pre_pos.sqrt()
        # pos_guide_perturbed = pos_guide # without conditional augmentation

        # Perterb pos - select noise for each node based on their graph
        a_pos = a.index_select(0, node2graph).unsqueeze(-1)  # (N, 1)
        pos_noise = torch.zeros(size=pos.size(), device=pos.device)
        pos_noise.normal_()
        pos_perturbed = pos + pos_noise * (1.0 - a_pos).sqrt() / a_pos.sqrt()

        # Update invariant edge features, as shown in equation 5-7, all the feature is invariant -> use to predict
        edge_inv_global, edge_inv_local, edge_index, edge_type, edge_length, local_edge_mask = self(
            atom_type = atom_type,
            pos = pos_perturbed,
            pos_guide = pos_guide_perturbed,
            bond_index = bond_index,
            bond_type = bond_type,
            batch = batch,
            time_step = time_step,
        ...
        train_edge_mask = is_train_edge(edge_index, is_sidechain)
        d_perturbed = torch.where(train_edge_mask.unsqueeze(-1), d_perturbed, d_gt)
        # select for train edge - train perturbed/else original
        ...
        # (E_global, 1), denoising direction, the true noise in distance

        global_mask = torch.logical_and(
                            torch.logical_or(d_perturbed <= self.config.cutoff, local_edge_mask.unsqueeze(-1)),
                            ~local_edge_mask.unsqueeze(-1)
                        ) # exclude local_edge_mask
        target_d_global = torch.where(global_mask, d_target, torch.zeros_like(d_target)) # global edge : extra virtual edge, if 2 atoms have a bond -> consider local edge
        edge_inv_global = torch.where(global_mask, edge_inv_global, torch.zeros_like(edge_inv_global))
        target_pos_global = eq_transform(target_d_global, pos_perturbed, edge_index, edge_length) # true noise / gradient for virtual global edge
        node_eq_global = eq_transform(edge_inv_global, pos_perturbed, edge_index, edge_length) # pred noise / denoise for virtual global edge
        loss_global = (node_eq_global - target_pos_global)**2
        loss_global = 2 * torch.sum(loss_global, dim=-1, keepdim=True)
    ...
    def langevin_dynamics_sample(self, atom_type, pos_init, pos_guide, bond_index, bond_type, batch, num_graphs, extend_order, extend_radius=True, 
                                 n_steps=100, step_lr=0.0000010, clip=1000, clip_local=None, clip_pos=None, min_sigma=0, is_sidechain=None,
                                 global_start_sigma=float('inf'), w_global=0.2, w_reg=1.0, **kwargs):
        return self.langevin_dynamics_sample_diffusion(atom_type, pos_init, pos_guide, bond_index, bond_type, batch, num_graphs, extend_order, extend_radius,
                        n_steps, step_lr, clip, clip_local, clip_pos, min_sigma, is_sidechain,
                        global_start_sigma, w_global, w_reg, 
                        sampling_type=kwargs.get("sampling_type", 'ddpm_noisy'), eta=kwargs.get("eta", 1.))
    
    def langevin_dynamics_sample_diffusion(self, atom_type, pos_init, pos_guide, bond_index, bond_type, batch, num_graphs, extend_order, extend_radius=True, 
                                 n_steps=100, step_lr=0.0000010, clip=1000, clip_local=None, clip_pos=None, min_sigma=0, is_sidechain=None,
                                 global_start_sigma=float('inf'), w_global=0.2, w_reg=1.0, **kwargs):
        ...
        sigmas = (1.0 - self.alphas).sqrt() / self.alphas.sqrt() # noise rate
        pos_traj = []
        if is_sidechain is not None:
            assert pos_gt is not None, 'need crd of backbone for sidechain prediction'
        with torch.no_grad():
            ...
            ## to test sampling with less intermediate diffusion steps
            # n_steps: the num of steps
            seq = range(self.num_timesteps-n_steps, self.num_timesteps) # T - 1 -> T - 1 - n
            seq_next = [-1] + list(seq[:-1])
            # x_t and x_t-1
            pos = pos_init * sigmas[-1] # corrupt to the C_t
            if is_sidechain is not None:
                pos[~is_sidechain] = pos_gt[~is_sidechain]
            
            # conditioning augmentation truncation timee
            # s = 5 #  hyperparameter- s=10 for 5000
            # eps_noise = torch.randn(size=pos_guide.size(), device=pos_guide.device)

            # pos_guide = pos_guide + eps_noise * (1.0 - self.alphas_p[s]).sqrt() / self.alphas_p[s].sqrt()
            ...
            for i, j in tqdm(zip(reversed(seq), reversed(seq_next)), desc='sample'):
                ...
                # + eps_pos_reg * w_reg, Gradient force has two component
                ...
                if sampling_type == 'generalized' or sampling_type == 'ddpm_noisy':
                    ...
                    if sampling_type == 'generalized':
                        ...
                        beta_t = 1 - at / atm1
                        e = -eps_pos
                        pos0_from_e = (1.0 / at).sqrt() * pos - (1.0 / at - 1).sqrt() * e # calculate pos_0 based on eps and t
                        mean_eps = (
                            (atm1.sqrt() * beta_t) * pos0_from_e + ((1 - beta_t).sqrt() * (1 - atm1)) * pos 
                        ) / (1.0 - at)
                        ## DDPM update # original calculation of mean no simplify version
                        mean = mean_eps
                        mask = 1 - (t == 0).float()
                        logvar = beta_t.log() # sigma_t = sqrt beta_t = exp 0.5* logbeta_t
                        pos_next = mean + mask * torch.exp(0.5 * logvar) * noise
                elif sampling_type == 'ld':
                    step_size = step_lr * (sigmas[i] / 0.01) ** 2
                    pos_next = pos + step_size * eps_pos / sigmas[i] + noise * torch.sqrt(step_size*2)
