...
class ConditionalGINEncoder(torch.nn.Module):
    ...
    def __init__(self, hidden_dim, num_convs=3, activation='relu', short_cut=True, concat_hidden=False):
        ...
        self.edge_transform = MultiLayerPerceptron(2 * hidden_dim, [hidden_dim, hidden_dim], activation=activation)
    ...
    def forward(self, z, edge_index, edge_attr):
        ...
        node_attr = self.node_emb(z)    # (num_node, hidden)
        edge_attr = self.edge_transform(edge_attr) # (E, 2 * hidden) -> (E， hidden) 
