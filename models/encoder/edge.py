...
class CoarseMLPEdgeEncoder(Module):

    def __init__(self, hidden_dim=100, activation='relu',  edge_type_num=4):
        super().__init__()
        self.hidden_dim = hidden_dim
        self.bond_emb = MultiLayerPerceptron(edge_type_num, [self.hidden_dim, self.hidden_dim], activation=activation)
        self.mlp = MultiLayerPerceptron(1, [self.hidden_dim, self.hidden_dim], activation=activation)
    ...
    def forward(self, edge_length, edge_type):
        ...
        Input:
            edge_length: The length of edges, shape=(E, 1).
            edge_type: The type pf edges, shape=(E, edge_type_num)
        Returns:
            edge_attr:  The representation of edges. (E, 2 * num_gaussians)
        """
        d_emb = self.mlp(edge_length) # (num_edge, hidden_dim)
...
def get_edge_encoder(cfg):
    ...
    elif cfg.edge_encoder == 'coarsemlp':
        return CoarseMLPEdgeEncoder(cfg.hidden_dim, cfg.mlp_act)
    else:
