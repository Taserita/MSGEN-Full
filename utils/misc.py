...
def repeat_data_from_condition(data, num_repeat, condition):
    datas = []
    condition = condition.reshape(-1, data.num_nodes, 3)

    condition_repeated = condition.repeat((num_repeat // condition.shape[0], 1, 1))[: num_repeat]
    for i in range(num_repeat):
        new_data = data.clone()
        new_data.condition = condition_repeated[i]
