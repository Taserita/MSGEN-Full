...
from utils.datasets import ConditionalConformationDataset
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ckpt', type=str, default=None, help='path for loading the stage-1 checkpoint')
    ...
    train_set = ConditionalConformationDataset(config.dataset.train, transform=transforms)
    val_set = ConditionalConformationDataset(config.dataset.val, transform=transforms)
    ...
    def train(it, num):
        ...
        # Graph information atom_type, bond_index, bond_type
        # How to use a pre-generate structure as guidance
        loss, loss_global, loss_local = model.get_loss(
            atom_type=batch.atom_type,
            pos=batch.pos,
            pos_guide=batch.condition,
    ...
    def save_checkpoint(it, model, optimizer_global, scheduler_global, optimizer_local, scheduler_local, avg_val_loss, best_val_loss, config, ckpt_dir):
        ...
        if avg_val_loss < best_val_loss:
            ...
            print(f"Best model updated at iteration {it} with loss {avg_val_loss:.4f}")
    ...
    try:
        ...
        for it in range(start_iter, config.train.max_iters + 1):
            ...
            if it % config.train.val_freq == 0 or it == config.train.max_iters:
                ...
                best_val_loss = save_checkpoint(it, model, optimizer_global, scheduler_global, optimizer_local, scheduler_local, avg_val_loss, best_val_loss, config, ckpt_dir)
